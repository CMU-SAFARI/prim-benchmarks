...
applications = {
    "VA": ["NR_DPUS=X NR_TASKLETS=Y BL=Z make all", "./bin/host_code -w 0 -e 1 -i #elements -x 0"],
            ...
            for r in NR_DPUS:
                for t in NR_TASKLETS:
                    for b in BL:
                        ...
                        if (app_name == "SpMV"):
                            if (r == 1):
                                r_cmd = run_cmd.replace("file_name",
                                                        "/")  # Include path to input file - Check SpMV/data/generate
                            if (r == 4):
                                r_cmd = run_cmd.replace("file_name",
                                                        "/")  # Include path to input file - Check SpMV/data/generate
                            if (r == 16):
