...
void create_test_file(unsigned int nr_elements) {
    ...
    A = (int32_t *)malloc(nr_elements * sizeof(int32_t));
    B = (int32_t *)malloc(nr_elements * sizeof(int32_t));
    C = (int32_t *)malloc(nr_elements * sizeof(int32_t));
...
//     end_region();
// }
static void vector_addition_host(unsigned int nr_elements) {
    // omp_set_num_threads(t);
    // #pragma omp parallel for
    start_region();
    for (int i = 0; i < nr_elements; i++) {
...
int main() {
    ...
    const unsigned int file_size = 4;
    ...
    vector_addition_host(file_size);
